...
export default function MascotasPage() {
  ...
  const rows = mascotas.map(m => ({
    ...
    tipo: m.tipo || '—',
    raza: m.raza || '—',
    residente: m.residente ? `${m.residente.nombre} ${m.residente.apellidos}` : '—',
    residencia: m.residente?.residencia ? `Residencia #${m.residente.residencia}` : '—'
  }));
    ...
    <div className="space-y-8">
      ...
      <SmartTable
        ...
        columns={[
          ...
          { key: "residente", label: "Propietario", enableSort: true },
      ...
      <ConfirmDialog
        ...
        message={`¿Seguro que deseas eliminar a "${deleteMascota?.nombre}"${deleteMascota?.residente ? ` (propietario: ${deleteMascota.residente.nombre} ${deleteMascota.residente.apellidos})` : ''}? Esta acción no se puede deshacer.`}
